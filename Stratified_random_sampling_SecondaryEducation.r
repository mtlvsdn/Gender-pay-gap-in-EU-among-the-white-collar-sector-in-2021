...
#URBAN POPULATION
urban <- data["F_secondary_educ"] #DATA URBAN
col_urban_west <- sample_west["F_secondary_educ"] #DATA WEST
col_urban_east <- sample_east["F_secondary_educ"] #DATA EAST
...
sample_west <- strata_west[-c(2,6,8,3,7,4,11,9,14), ] #WE CHOOSE A SAMPLE FOR WEST STRATA RANDOMLY
sample_east <- strata_east[-c(11), ] #WE CHOOSE A SAMPLE FOR EAST STRATA RANDOMLY
sample_west_urban <- sample_west["F_secondary_educ"]
sample_east_urban <- sample_east["F_secondary_educ"]
...
plotCI(
  ...
  main = "Confidence Intervals for \nSecondary Education (in %) among women\n in Western and Eastern EU in 2021"
)
