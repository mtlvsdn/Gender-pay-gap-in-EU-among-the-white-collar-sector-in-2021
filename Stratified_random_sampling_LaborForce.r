...
#URBAN POPULATION
urban <- data["F_Labour_force"] #DATA URBAN
col_urban_west <- sample_west["F_Labour_force"] #DATA WEST
col_urban_east <- sample_east["F_Labour_force"] #DATA EAST
...
sample_west <- strata_west[-c(10,13,5,4,12,7,6,2,9), ] #WE CHOOSE A SAMPLE FOR WEST STRATA RANDOMLY
sample_east <- strata_east[-c(13,7,8,12,1,10,4,9,3), ] #WE CHOOSE A SAMPLE FOR EAST STRATA RANDOMLY
sample_west_urban <- sample_west["F_Labour_force"]
sample_east_urban <- sample_east["F_Labour_force"]
...
mean_west <- mean(sample_west_urban, na.rm = TRUE) #MEAN WEST
...
hist(sample_west_urban, breaks = 1, main = "Distribution of Labor Force (in %)\n among women in Western EU in 2021", xlab = "Urbanization Rate", col = 'lightblue')
View(sample_west_urban)
# DISTRIBUTION OF SAMPLE EAST
View(sample_east_urban)
hist(sample_east_urban, breaks = 4, main = "Distribution of Labor Force (in %)\n among women in Eastern EU in 2021", xlab = "Urbanization Rate", col = 'orange')
...
plotCI(
  ...
  main = "Confidence Intervals for Labor Force (in %)\n among women in Western and Eastern EU in 2021"
)

plotCI(
  x = 1:2,                   # X-axis positions of the points
  y = mean_values,           # Mean values
  uiw = 2 * standard_errors, # Widths of the confidence intervals (twice the standhttp://127.0.0.1:21375/graphics/ac0d7f90-7e3c-4871-ac78-fd73b3116efb.pngard errors)
  ...
  main = "Confidence Intervals for Labor Force (in %)\n among women in Western and Eastern EU in 2021",  # Main plot title
