...
#URBAN POPULATION
urban <- data["Financial"] #DATA URBAN
col_urban_west <- sample_west["Financial"] #DATA WEST
col_urban_east <- sample_east["Financial"] #DATA EAST
...
sample_west <- strata_west[-c(11,14,3,9,5,13), ] #WE CHOOSE A SAMPLE FOR WEST STRATA RANDOMLY
sample_east <- strata_east[-c(11,13,4,1,8,5,2,9,12), ] #WE CHOOSE A SAMPLE FOR EAST STRATA RANDOMLY
sample_west_urban <- sample_west["Financial"]
sample_east_urban <- sample_east["Financial"]
...
hist(sample_west_urban, breaks = 7, main = "Distribution of Pay Gap in the \nFinancial sector\n in Western EU in 2021", xlab = "Urbanization Rate", col = 'lightblue')
View(sample_west_urban)
